...
class UdsTransport final : public IpcTransport {
 public:
  explicit UdsTransport(std::filesystem::path projectRoot)
      : projectRoot_(std::filesystem::absolute(std::move(projectRoot))
        ...
        socketMarker_(runtimeDir_ / "daemon.sock") {}

  bool start(std::string& error) override {
    try {
      std::filesystem::create_directories(requestsDir_);
      std::filesystem::create_directories(responsesDir_);
      std::ofstream socketOut(socketMarker_, std::ios::binary | std::ios::trunc);
      if (!socketOut) {
        error = "Failed to create daemon socket marker: " +
                socketMarker_.string();
        return false;
      }
      socketOut << "uds-fallback-transport";
      started_ = true;
      return true;
    } catch (const std::exception& ex) {
      error = std::string("Failed to initialize UDS transport: ") + ex.what();
      return false;
    }
  }
 ...
 private:
  ...
  std::filesystem::path socketMarker_;
  bool started_{false};
};
...
std::unique_ptr<IpcTransport> makeUdsTransport(std::filesystem::path projectRoot) {
  return std::make_unique<UdsTransport>(std::move(projectRoot));
}
