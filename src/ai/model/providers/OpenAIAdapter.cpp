...
bool OpenAIAdapter::initialize(const nlohmann::ordered_json& config,
  ...
  info_.modelName = defaultConfiguredModelName(config_, "gpt-4o-mini");
...
ModelResponse OpenAIAdapter::generate(const ModelRequest& request) {
  ...
  const std::string baseUrl =
      config_.value("base_url", std::string{"https://api.openai.com"});
  ...
  if (!httpResp.ok) {
    if (httpResp.statusCode == 0) {
      return makeError(ModelErrorCode::ProviderUnavailable,
                       "OpenAI HTTP transport error: " + httpResp.errorMessage);
    }
    try {
    ...
    return makeError(ModelErrorCode::ProviderUnavailable,
                     "OpenAI returned HTTP " + std::to_string(httpResp.statusCode));
  }
  ...
  try {
    ...
    return makeError(ModelErrorCode::InvalidResponse,
                     std::string("OpenAI response JSON parse failed: ") + ex.what());
  }
}
