...
bool DeepSeekAdapter::initialize(const nlohmann::ordered_json& config,
                                 std::string& error) {
  if (!config.is_null() && !config.is_object()) {
    error = "DeepSeek adapter configuration must be an object.";
    initialized_ = false;
    return false;
  }
  ...
  if (!hasApiKey && !hasMockResponse) {
    error = "DeepSeek adapter requires 'api_key' or 'mock_response'.";
    initialized_ = false;
    return false;
  }
...
ModelResponse DeepSeekAdapter::generate(const ModelRequest& request) {
  if (!initialized_) {
...
std::vector<std::string> DeepSeekAdapter::tokenize(const std::string& text) const {
  return tokenizeWhitespace(text);
}

ModelInfo DeepSeekAdapter::modelInfo() const {
  return info_;
}

void DeepSeekAdapter::shutdown() {
  initialized_ = false;
}

nlohmann::ordered_json DeepSeekAdapter::buildProviderRequest(
...
ModelResponse DeepSeekAdapter::translateResponse(
  ...
  if (!providerResponse.is_object() || !providerResponse.contains("choices") ||
    ...
    return makeError(ModelErrorCode::InvalidResponse,
                     "DeepSeek response must contain non-empty array 'choices'.");
  }
  ...
  if (!choice.is_object() || !choice.contains("message") ||
    ...
    return makeError(ModelErrorCode::InvalidResponse,
                     "DeepSeek response is missing message payload.");
  }
  ...
  const std::string reasoningContent =
      message.value("reasoning_content", std::string{});

  ModelResponse response;
  response.ok = true;
  response.textOutput = content.empty() ? reasoningContent : content;
  if (!content.empty() && !reasoningContent.empty() && reasoningContent != content) {
    response.textOutput = reasoningContent + "\n\n" + content;
  }
  ...
  if (message.contains("tool_calls")) {
    if (!parseToolCalls(message.at("tool_calls"), response.toolCalls)) {
      return makeError(ModelErrorCode::InvalidResponse,
                       "DeepSeek tool call payload is invalid.",
                       response.latencyMs);
    }
  }
